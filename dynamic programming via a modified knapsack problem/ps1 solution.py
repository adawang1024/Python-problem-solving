...
def election_winner(election):
    ...
    for state in election:
         ...
         # get the ec vote of the winner states
         if winner == "dem": 
...
def ec_votes_to_flip(election, total=538):
    ...
    #number of additional EC votes needed by the loser = number of EC votes that should lost by the winner

    return int(winner_ECvotes -(total/2 - 1))
    ...
    * A tuple containing the empty list followed by zero, if every state has a # EC votes greater
    ...
    def helper(winner_states, max_ec_votes, memo = None):
          ...
          if (len(winner_states), max_ec_votes) in memo:
              ...
              result = helper(winner_states[1:], max_ec_votes, memo)
          else:
              next_item = winner_states[0]
              #Explore left branch
              with_to_take, with_val =\
                        helper(winner_states[1:],
                                  max_ec_votes - next_item.get_ecvotes(), memo)
             
              with_val += next_item.get_margin()+1
              #Explore right branch
              without_to_take, without_val  = helper(winner_states[1:],
                                                      max_ec_votes, memo)
              #Choose better branch
              if with_val > without_val:
                  result = (with_to_take + [next_item],with_val)
              else:
                  result = (without_to_take,without_val)
          memo[(len(winner_states), max_ec_votes)] = result
          return result
      
    ans = helper(winner_states, max_ec_votes, memo = None)
    return ans
...
def min_voters_moved(winner_states, ec_votes_needed):
    ...
    * * A tuple containing the empty list followed by zero, if no possible swing states.
    ...
    for i in winner_states:
        winner_ECvotes += i.get_ecvotes()   
    votes_needed = winner_ECvotes - ec_votes_needed
    print(votes_needed)
    print(ec_votes_needed)
    # gets the flipping stats and voters
    states, voters = max_voters_moved(winner_states,votes_needed)
    ans_state = []
    # subtract  the flipping states from the total tsates
    for i in winner_states:
        if i not in states: 
            ans_state.append(i)
    voters = 0
    for i in ans_state:
        voters += i.get_margin() + 1
    return (ans_state,voters)
...
def relocate_voters(election, swing_states, ideal_states = ['AL', 'AZ', 'CA', 'TX']):
    ...
    * None, if it is not possible to sway the election
    """
    # Find the list of states that were won by the election loser and that are not ideal states
    loser_states = []
    ...
    for i in loser_states: 
        if i.name not in ideal_states: 
            losing_candidate_states.append(i)
    print(losing_candidate_states)
    print(ideal_states)
    total_voters = 0
    total_ec = 0
    for swing_state in swing_states:
        relocated_voter = swing_state.get_margin()+1
        total_voters += relocated_voter
        total_ec += swing_state.get_ecvotes()
        while losing_candidate_states:
            # when the current states fulfill the requirement of voters, break
            if relocated_voter == 0 :
                break
            losing_state = losing_candidate_states[0]
            # can not relocate more than state.get_margin()-1 voters from losing state
            available_voter = losing_state.get_margin()-1
            if available_voter > relocated_voter:
                losing_state.add_losing_candidate_voters(relocated_voter)
                voters_dict[(losing_state.name, swing_state.name)] = relocated_voter
                relocated_voter = 0 
            else:
               
               relocated_voter -= available_voter
               losing_candidate_states.remove(losing_state)
               voters_dict[(losing_state.name,swing_state.name)] = available_voter
               # remove the states alreaydy used
    ...
    return (total_voters, total_ec,voters_dict)
...
if __name__ == "__main__":
    ...
    reqd_ec_votes = ec_votes_to_flip(election) # reqd_ec_votes = ec_votes_to_flip(election)-1 calcultaes the EC needs for a tie
    print("Winner:", winner, "\nLoser:", loser)
