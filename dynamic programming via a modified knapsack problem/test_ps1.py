...
def check_valid_mapping(election, move_voters_result, expected_results):
    ...
    ideal_states = ['AL', 'AZ', 'CA', 'TX']
    winner_won_states = ps1.winner_states(election)

    for state_from, state_to in voter_map:
        assert state_from not in ideal_states, f"Your mapping should not have moved voters from an ideal state ({state_from}), but it turns out it has."
...
class TestPS1(unittest.TestCase):
    def test_1_load_election(self):
        ...
        parsed_election = ps1.load_election("60001_results.txt")
    ...
    def test_2b_winner_states(self):
        ...
        winners = [state.get_name() for state in ps1.winner_states(ps1.load_election("60001_results.txt"))]
        self.assertIsInstance(winners, list, f"winner_states should have returned a list, but instead returned an instance of {type(winners)}.")
        self.assertEqual(len(winners), len(set(winners)), "winner_states should have returned a list with no duplicates, but found it had duplicates.")
    ...
    def test_2c_ec_votes_to_flip(self):
        ...
        # 60001_results.txt
        expected_ecvotes = 2
        ecvotes = ps1.ec_votes_to_flip(ps1.load_election("60001_results.txt"), total=14)
        self.assertIsInstance(ecvotes, int, f"ec_votes_to_flip should have returned an int, but instead returned an instance of {type(ecvotes)}.")
    ...
    def test_3_brute_swing_states(self):
        ...
        election = ps1.load_election("600_results.txt")
        loser_lost_states = ps1.winner_states(election)
        loser_ecvotes_reqd = ps1.ec_votes_to_flip(election)
        ...
        election = ps1.load_election("60002_results.txt")
        loser_lost_states = ps1.winner_states(election)
        loser_ecvotes_reqd = ps1.ec_votes_to_flip(election, total=14)
        raw_swing_states = ps1.brute_force_swing_states(copy.deepcopy(loser_lost_states), loser_ecvotes_reqd)
        swing_states, voters_moved = raw_swing_states
        swing_names = [state.get_name() for state in swing_states]
        actual_output = set(swing_names), voters_moved
        ...
        election = ps1.load_election("2020_results_brute.txt")
        loser_lost_states = ps1.winner_states(election)
        loser_ecvotes_reqd = ps1.ec_votes_to_flip(election, total=122)
        raw_swing_states = ps1.brute_force_swing_states(copy.deepcopy(loser_lost_states), loser_ecvotes_reqd)
        swing_states, voters_moved = raw_swing_states
        swing_names = [state.get_name() for state in swing_states]
        actual_output = set(swing_names), voters_moved
        ...
        election = ps1.load_election("2016_results_brute.txt")
        loser_lost_states = ps1.winner_states(election)
        loser_ecvotes_reqd = ps1.ec_votes_to_flip(election, total=44)
        raw_swing_states = ps1.brute_force_swing_states(copy.deepcopy(loser_lost_states), loser_ecvotes_reqd)
        swing_states, voters_moved = raw_swing_states
        swing_names = [state.get_name() for state in swing_states]
        actual_output = set(swing_names), voters_moved
        ...
        election = ps1.load_election("2012_results_brute.txt")
        loser_lost_states = ps1.winner_states(election)
        loser_ecvotes_reqd = ps1.ec_votes_to_flip(election, total=98)
        raw_swing_states = ps1.brute_force_swing_states(copy.deepcopy(loser_lost_states), loser_ecvotes_reqd)
        swing_states, voters_moved = raw_swing_states
        swing_names = [state.get_name() for state in swing_states]
        actual_output = set(swing_names), voters_moved
    ...
    def test_4a_move_max_voters(self):
        ...
        election = ps1.load_election("2020_results.txt")
        loser_lost_states = ps1.winner_states(election)
        loser_ecvotes_reqd = ps1.ec_votes_to_flip(election)
        ...
        election = ps1.load_election("2016_results.txt")
        loser_lost_states = ps1.winner_states(election)
        loser_ecvotes_reqd = ps1.ec_votes_to_flip(election)
        loser_ecvotes = sum(state.get_ecvotes() for state in loser_lost_states)
        raw_moved_max_voters = ps1.max_voters_moved(loser_lost_states, loser_ecvotes-loser_ecvotes_reqd)
        ...
        election = ps1.load_election("2012_results.txt")
        loser_lost_states = ps1.winner_states(election)
        loser_ecvotes_reqd = ps1.ec_votes_to_flip(election)
        loser_ecvotes = sum(state.get_ecvotes() for state in loser_lost_states)
        raw_moved_max_voters = ps1.max_voters_moved(loser_lost_states, loser_ecvotes-loser_ecvotes_reqd)
    ...
    def test_5_relocate_voters(self):
        ...
        results_dp = ps1.relocate_voters(copy.deepcopy(election), copy.deepcopy(swing_states[0]))
        self.assertTrue(check_valid_mapping(election, results_dp, example), f"Your relocate_voters results did not give the correct result.\nFor the sample election you got {results_dp}\nOne valid solution is {example}.")
