...
class DirectedRoad():
    ...
    def get_travel_time(self, has_traffic=False):
        ...
        """
        if has_traffic == True:
            total_time = self.travel_time*self.traffic_multiplier
            return total_time
        else:
    ...
    def __str__(self):
        ...
        return "%s -> %s takes %s minute(s) via %s road with traffic multiplier %s" % (self.src_node, self.dest_node, self.travel_time, self.road_type,self.traffic_multiplier)
...
class RoadMap():
    ...
    def contains_node(self, node):
        ...
        if node in self.nodes and node in self.nodes_to_roads.keys(): 
            return True
    ...
    def insert_node(self, node):
        ...
        Parameter:
            node: Node
                Node to add.
        """
        if node in self.nodes or node in self.nodes_to_roads:
            raise ValueError 
        self.nodes.add(node)
    ...
    def insert_road(self, road):
        ...
        destination = road.get_destination_node()
        # print(source)
        # print(destination)
        if not (self.contains_node(source) and self.contains_node(destination)):
           raise ValueError
